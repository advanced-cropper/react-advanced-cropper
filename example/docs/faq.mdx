...
## I want to add the additional setting to the cropper. How to do it?
...
you can use generic nature of cropper components:
```tsx
import { CropperSettings } from 'react-advanced-cropper';

interface CustomSettings extends CropperSettings {
	stencilType: string;
}
```
```tsx
