...
Adjust stencil is the one of [`scaleImage`](/components/cropper.html#resizeimage) prop option.
...
export const Example = () => {
	const percentsRestriction = (state: CropperState, settings: CoreSettings) => {
	...
	return (
		<Cropper
			sizeRestrictionsAlgorithm={percentsRestriction}
