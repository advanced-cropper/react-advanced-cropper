...
import { DefaultPositionExample } from '../../src/components/examples/DefaultPositionExample';
...
Sometimes you should set the default position and default size of cropper. For example, if you automatically detect an user face.

<DefaultPositionExample/>
...
export const Example = () => {
	const defaultPosition = (state: CropperState, settings: CoreSettings) => {
			return {
				left: 100,
				top: 100,
			};
	},
	const defaultSize = (state: CropperState, settings: CoreSettings) => {
			return {
				width: 400,
				height: 400,
			};
...
The `default-size` prop can determine the default size based on the `state`  as mentioned in [its documentation](/docs/components/Cropper#defaultsize).
...
There are two methods to move and scale image: **move** and **scale**.
...
export const Example = () => {
	...
	const zoom = () => {
		if (cropperRef.current) {
			cropperRef.current.zoom(2); // zoom-in 2x
		}
	};
	const move = () => {
		if (cropperRef.current) {
			cropperRef.current.move(50, 100); // move x = 50, y = 100
		}
	};
	return (
		<Cropper
...
```js
import React from 'react';
import { Cropper, CropperRef, DefaultSettings, retrieveSizeRestrictions, getTransformedImageSize } from 'react-advanced-cropper';
...
The source code of this example is available in [the sandbox](https://codesandbox.io/s/vue-advanced-cropper-custom-scroll-eyidfn?file=/src/index.tsx).
