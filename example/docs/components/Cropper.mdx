...
- **Type:** `Size | ((state: CropperState, settings: CropperSettings) => Size)`

- **Details:**

	The size of the stencil in pixels (absolute, not relative to an image).
	...
	**See also: [Example](/docs/guides/advanced-recipes#fixed-stencil)**
...
- **Type:** `boolean | ScaleImageSettings`
```tsx
interface ScaleImageSettings {
	touch?: boolean;
	wheel?:
    | boolean
    | {
...
- **Details:**
	...
	This prop is either boolean flag or object settings. The default object used when `true` is passed to `scaleImage`:
	```js
	{
    touch: true,
    wheel: {
...
- **Type:** `boolean | MoveImageSettings`
```tsx
interface MoveImageSettings {
	touch?: boolean;
	mouse?: boolean;
}
```
...
- **Type:** `boolean | RotateImageSettings`
```tsx
interface RotateImageSettings {
	touch?: boolean;
}
```
...
- **Details:**
	...
	The `touch` field checks if image should be rotated by a rotate gesture.
...
- **Details:**
	...
	The escape hatch to pass the additional parameters to `CropperSettings`, that could be used
	by the custom algorithms.
...
- **Type:** `Position | ((state: CropperState, settings: CropperSettings) => Position);`
...
- **Type:** `SizeRestrictions | ((state: CropperState, settings: ExtendedCropperSettings) => SizeRestrictions)`
...
- **Type:** `PositionRestrictions | ((state: CropperState, settings: CropperSettings) => PositionRestrictions)`
...
- **Type:** `(state: CropperState, settings: CropperSettings, , transforms: CoordinatesTransform | CoordinatesTransform[], safe?: boolean) => CropperState`
...
- **Type:** `(state: CropperState, settings: CropperSettings, visibleArea: VisibleArea, safe?: boolean) => CropperState`
...
- **Type:** `(state: CropperState, settings: CropperSettings, boundary: Boundary) => CropperState`
...
- **Type:** `(state: CropperState, settings: CropperSettings, move?: MoveDirections, scale?: Scale | number) => CropperState`
```tsx
...
- **Type:** `(state: CropperState, settings: CropperSettings, directions: ResizeDirections, options: ResizeOptions) => CropperState`
...
- **Type:** `(options: CreateStateParams, settings: CropperSettings) => CropperState`
...
- **Type:** `(state: CropperState, settings: CropperSettings) => CropperState`
...
- **Type:** `(state: CropperState, settings: CropperSettings, directions: MoveDirections) => CropperState`
```tsx
interface MoveDirections {
    top?: number;
    left?: number;
...
- **Type:** `(state: CropperState, settings: CropperSettings, horizontal?: boolean, vertical?: boolean) => CropperState`
...
- **Type:** `(state: CropperState, settings: CropperSettings, angle: number) => CropperState`
...
- **Type:** `(state: CropperState, settings: CropperSettings, scale: Scale | number) => CropperState`
```tsx
interface Scale {
    factor: number;
    center?: Point;
...
- **Type:** `(state: CropperState, settings: CropperSettings, move: MoveDirections) => CropperState`
```tsx
interface MoveDirections {
    top?: number;
    left?: number;
...
- **Returns:**
	...
	The actual [cropper state](/docs/concept/basic):
	```tsx
...
- **Returns:**
	...
	The settings for the cropper. By default, it has `CropperSettings` type, but it can contain additional custom fields depends of
	passed settings.
...
- **Arguments:**
  ...
  `transform: CoordinatesTransform | CoordinatesTransform[]`
    ...
    ((state: CropperState, settings: CropperSettings) => Partial<Coordinates>) | Partial<Coordinates>;
	```
...
Callback that called on change.
