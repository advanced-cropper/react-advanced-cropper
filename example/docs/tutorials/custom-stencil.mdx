...
- if stencil has aspect ratios it should set the stencil option `aspectRatio` to inform the cropper resize algorithm about it
...
The stencil should inform the cropper that he has aspect ratio limitation. There is the `cropper.setStencilOptions` method for this.
You should pass the object with the following structure:
```
{
	aspectRatio: number | {
		minimum?: number;
...
This method should be called on each options change and on component mount, so it's naturally to use `useEffect` hook for that.
