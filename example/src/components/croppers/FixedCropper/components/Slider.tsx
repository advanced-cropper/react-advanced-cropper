...
export class Slider extends PureComponent<Props> {
	...
	render() {
		...
		return (
			<div className={cn('fixed-cropper-slider', className)} ref={this.line}>
				<div className="fixed-cropper-slider__line">
					<div
						className="fixed-cropper-slider__fill"
						style={{
							flexGrow: value,
						}}
					/>
					...
					>
						<div
							className={cn(
								'fixed-cropper-slider__inner-circle',
								this.state.focus && 'fixed-cropper-slider__inner-circle--focus',
							)}
						/>
					</div>
				</div>
